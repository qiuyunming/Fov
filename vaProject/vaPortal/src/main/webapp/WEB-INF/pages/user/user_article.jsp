...
<body>
								...
								<div class="panel-heading">
									<div>
										<h2 class="panel-title pull-left">
											<a href="<%=request.getContextPath()%>/community/article/${item.id}">${item.title}</a>
										</h2>
										<button class="button button-tiny button-box pull-right" onclick="delete_article(${item.id})">
											<i class="icon icon-trash"></i>
										</button>
										<button class="button button-tiny button-box pull-right" onclick="location.href='<%=request.getContextPath()%>/user/update_article/${item.id}'">
											<i class="icon icon-wrench"></i>
										</button>
									</div>
									<div class="clearfix"></div>
								</div>
								<div class="panel-body" onclick="location.href='<%=request.getContextPath()%>/community/article/${item.id}'" style="cursor: pointer">
	...
	var delete_article = function(id){
		$.ajax({
			url:"<%=request.getContextPath()%>/user/delete_article",
			type:"POST",
			data:{"id":id},
